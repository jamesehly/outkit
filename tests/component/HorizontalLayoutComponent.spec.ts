...
describe('HorizontalLayoutComponent Tests', () => {
    ...
    it('should add each child element as a component and have fluid children', (done) => {
        ...
        sut.relay('init').then(() => {
            ...
            expect(children[0].getElement().offsetHeight).toBeCloseTo(document.body.offsetHeight, 1);
            expect(children[1].getElement().offsetHeight).toBeCloseTo(document.body.offsetHeight, 1);
            expect(children[2].getElement().offsetHeight).toBeCloseTo(document.body.offsetHeight, 1);
            done();
        })
    });
